...
export async function post_process(result_scope, container, params = {}) {
  ...
  const env = item.env;
  const plugin = /** @type {import('jsbrains/smart-types').ConnectionsPlugin} */ (env.smart_connections_plugin);
  ...
  const render_result = async (_result_elm) => {
    if (!_result_elm.querySelector('li').innerHTML) {
      const collection_key = /** @type {import('jsbrains/smart-types').ConnectionsCollectionKey} */ (_result_elm.dataset.collection);
      const entity = /** @type {import('jsbrains/smart-types').ConnectionItem} */ (
  ...
  plugin.registerDomEvent(container, 'contextmenu', (event) => {
    ...
    const connections_list = /** @type {import('jsbrains/smart-types').ConnectionsListScope} */ (result_scope.connections_list);
 ...
 * @param {import('jsbrains/smart-types').ConnectionItem} item
 * @param {import('jsbrains/smart-types').ConnectionsComponentSettings} [component_settings]
 * @returns {string}
 */
...
/**
 * @param {import('jsbrains/smart-types').ConnectionItem|null|undefined} entity
 * @returns {boolean}
 */
