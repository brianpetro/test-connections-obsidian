...
/** @typedef {{get?: (key: string) => unknown, items?: Object.<string, unknown>, fs?: import('jsbrains/smart-types/index.js').ConnectionsFs}} ConnectionsTargetCollection */
/** @typedef {Record<string, unknown> & {smart_sources?: ConnectionsTargetCollection, smart_blocks?: ConnectionsTargetCollection, fs?: import('jsbrains/smart-types/index.js').ConnectionsFs}} DroppedConnectionsEnv */


/**
 * @param {DroppedConnectionsEnv} env
 * @param {import('jsbrains/smart-types/index.js').ConnectionsCollectionKey} collection_key
...
/**
 * @param {unknown} item
 * @returns {item is import('jsbrains/smart-types/index.js').ConnectionItem}
 */
function is_connections_target(item) {
  const candidate = /** @type {Partial<import('jsbrains/smart-types/index.js').ConnectionItem>|null|undefined} */ (item);
...
function get_smart_targets(env, data_transfer) {
  ...
  for (const { collection_key, item_key } of smart_drag_data.items) {
    ...
    const target = get_collection_item(
      env,
      /** @type {import('jsbrains/smart-types/index.js').ConnectionsCollectionKey} */ (collection_key),
      item_key,
    );
...
function get_native_targets(env, data_transfer) {
  ...
  const source_items = /** @type {Array<Partial<import('jsbrains/smart-types/index.js').ConnectionItem>>} */ (
