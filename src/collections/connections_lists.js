...
export class ConnectionsLists extends Collection {
   ...
   * @returns {import('jsbrains/smart-types').ConnectionsListScope}
   */
  new_item(item) {
  ...
  get connections_list_component_settings_config() {
    ...
    const config = typeof component_module?.settings_config === 'function'
      ? /** @type {(scope: ConnectionsListsCollection) => import('jsbrains/smart-types').SettingsConfig} */ (
        component_module.settings_config
      )(this)
      : /** @type {import('jsbrains/smart-types').SettingsConfig|undefined} */ (component_module?.settings_config)
    ;
